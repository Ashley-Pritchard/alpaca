	...
	def get(self, request):
		...
		if len(set(input_list))==2:
			...
			if form.is_valid():
				...
				if request.GET['region_input']:
					

					region = form.cleaned_data['region_input']

					start = int(region.split('_')[0])
					stop = int(region.split('_')[1])


					variants = Variant.objects.all().values()

				
					variants_df = pd.DataFrame(list(variants))

					variants_df = variants_df[~variants_df['g_name'].str.contains('_')]

					variants_df['coordinate'] = variants_df['g_name'].apply(lambda x: int(''.join(filter(str.isdigit, x))))

					variants_df = variants_df[variants_df['coordinate'].between(start, stop)]
